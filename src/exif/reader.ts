...
export function parseIfd(
  ...
  try {
    ...
    const nextIfdOffset = nextPtr + 4 <= data.length ? dataview.readUint32(data, nextPtr, le) : 0;
 ...
 */
export function readEntryValue(data: Uint8Array, entry: IfdEntry, le: boolean): unknown {
  ...
  switch (type) {
    case 2 /* ASCII */: {
      if (inline) {
        return readAscii(rawValueBytes, 0, count);
      }
    ...
    case 3 /* SHORT */: {
      if (count === 1) {
        return inline
          ? readShortInline(rawValueBytes, le)
          : dataview.readUint16(data, valueOffset, le);
      }
      const arr: number[] = [];
    ...
    case 4 /* LONG */: {
      if (count === 1) {
        return inline
          ? dataview.readUint32(rawValueBytes, 0, le)
          : dataview.readUint32(data, valueOffset, le);
      }
      const arr: number[] = [];
      for (let i = 0; i < count; i++) {
        if (valueOffset + i * 4 + 4 > data.length) {
          break;
        }
 ...
 */
export function parseIfdValues(
  data: Uint8Array,
  ifdOffset: number,
  le: boolean
): Map<number, unknown> {
...
export function parseGpsIfd(
  ...
  if (speedRaw && speedRaw[1] !== 0) {
    result.speed = speedRaw[0] / speedRaw[1];
  }

  const dirRaw = tags.get(17) as [number, number] | undefined;
  if (dirRaw && dirRaw[1] !== 0) {
    result.direction = dirRaw[0] / dirRaw[1];
  }
...
export function parseExifIfd(data: Uint8Array, exifOffset: number, le: boolean): ExifData {
  ...
  const dto = str(36867);
  if (dto) {
    ex.dateTimeOriginal = dto;
  }
  const dtd = str(36868);
  if (dtd) {
    ex.dateTimeDigitized = dtd;
  }

  const et = rat(33434);
  if (et && et[1] !== 0) {
    ex.exposureTime = et[0] === 1 ? `1/${et[1]}` : `${et[0]}/${et[1]}`;
  }

  const fn = rat(33437);
  if (fn && fn[1] !== 0) {
    ex.fNumber = Math.round((fn[0] / fn[1]) * 10) / 10;
  }
  ...
  const fl = rat(37386);
  if (fl && fl[1] !== 0) {
    ex.focalLength = Math.round(fl[0] / fl[1]);
  }

  const flash = num(37385);
  if (flash !== undefined) {
    ex.flash = (flash & 0x1) === 1;
  }
  ...
  const lmake = str(42035);
  if (lmake) {
    ex.lensManufacturer = lmake;
  }
...
export function readExifBlock(exifData: Uint8Array, out: Partial<MetadataMap>): void {
  ...
  try {
    ...
    if (dataview.readUint16(exifData, 2, le) !== 42) {
      return;
    } // TIFF magic
    ...
    if (exifPtr) {
      out.exif = parseExifIfd(exifData, exifPtr, le);
    }
