/**
 * HB_Scrub GUI — embedded HTML/CSS/JS single-page application.
 * This file is the only place the UI markup and styling live.
 * Kept separate from server logic to reduce cognitive load when editing either side.
 */
...
<body>
  ...
  (function() {
    ...
    // ── HTML escape helper — use for all user-controlled data in innerHTML ──
    function esc(s) {
      return String(s)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
    ...
    function setBanner(kind, title, detail) {
      ...
      statusBanner.innerHTML = '<div><strong>' + esc(title) + '</strong><span>' + esc(detail) + '</span></div>';
    }
    ...
    function renderRecentList(target, items, emptyLabel) {
      ...
      target.innerHTML = items.map(item => '<li class="history-item"><strong>' + esc(item.label) + '</strong><small>' + esc(new Date(item.at).toLocaleString()) + '</small></li>').join('');
    }
    ...
    function renderMetaTags(id) {
      ...
      for (const t of allTypes) {
        if (removedSet.has(t)) {
          html += \`<span class="meta-tag" style="text-decoration:line-through;opacity:0.45" title="removed">\${esc(t)}</span>\`;
        } else {
          html += \`<span class="meta-tag">\${esc(t)}</span>\`;
        }
      }
    ...
    function renderHistory() {
      ...
      historyList.innerHTML = items.map(item => {
        const when = esc(new Date(item.at).toLocaleString());
        const filesLabel = esc(item.files + ' file' + (item.files !== 1 ? 's' : ''));
        return '<li class="history-item">'
          + '<strong>' + filesLabel + '</strong>'
          + '<span>' + esc(String(item.cleaned)) + ' cleaned \u2022 ' + esc(item.savedText) + ' saved</span>'
          + '<small>' + when + (item.errors ? ' \u2022 ' + esc(String(item.errors)) + ' issues' : '') + '</small>'
          + '</li>';
      }).join('');
    }
